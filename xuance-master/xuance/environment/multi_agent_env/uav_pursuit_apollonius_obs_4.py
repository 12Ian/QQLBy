...
class UAVPursuitApolloniusObs4Env(RawMultiAgentEnv):
    def __init__(self, config):
        super(UAVPursuitApolloniusObs4Env, self).__init__()
    ...
    def _generate_city_blocks(self):
        import json
        import os
        difficulty = "open"  # 你可以根据需要修改为 "easy", "medium", "hard"
        # 请根据你的实际路径修改
    ...
    def _assign_target_points(self):
        ...
        for idx, angle in enumerate(angles):
            ...
            for b in self.buildings:
                xmin, xmax, ymin, ymax = b
                tx1 = (xmin - safe_margin - self.target_position[0]) / (dx + 1e-8)
                tx2 = (xmax - safe_margin - self.target_position[0]) / (dx + 1e-8)
                ty1 = (ymin - safe_margin - self.target_position[1]) / (dy + 1e-8)
                ty2 = (ymax - safe_margin - self.target_position[1]) / (dy + 1e-8)
        ...
        if len(open_indices) == 0:
            ...
            for i, sector in enumerate(sectors):
                ...
                if num_for_sector > 0:
                    ...
                    # 计算 \delta 冗余角，并限制其不超过扇区宽度的 1/3
                    delta = np.arcsin(np.clip(self.catch_radius / (r_f + 1e-5), 0, 1)) * 0.8
                    delta = min(delta, sector_width / 3.0)
                    
                    if num_for_sector == 1:
                        angles_to_use = [(start_angle + end_angle) / 2]
                    else:
                        angles_to_use = np.linspace(start_angle + delta, end_angle - delta, num_for_sector)
