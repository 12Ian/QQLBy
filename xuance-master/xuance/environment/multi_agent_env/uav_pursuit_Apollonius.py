...
class UAVPursuitApolloniusObs2Env(RawMultiAgentEnv):
    def __init__(self, config):
        super(UAVPursuitApolloniusObs2Env, self).__init__()
        ...
        # [修改点 1]: obs_dim 变为 32
        # 构成: pos(2) + yaw_vec(2) + norm_speed(1) + rel_target(2) + rel_mates(6) + radar(16) + target_yaw_vec(2) + target_speed(1) = 32
        obs_dim = 32
        self.observation_space = {agent: gym.spaces.Box(low=-1.0, high=1.0, shape=(obs_dim,), dtype=np.float32) for
                                  agent in self.agents}
        
        # [修改点 2]: state_space 变为 20，补上了 target_speed
        # 构成: uav_pos(8) + uav_yaws(4) + uav_speeds(4) + target_pos(2) + target_yaw(1) + target_speed(1) = 20
        self.state_space = gym.spaces.Box(low=-np.inf, high=np.inf, shape=(20,), dtype=np.float32)
        ...
        self.last_encircle_potential = 0.0
    ...
    def _get_obs(self):
        ...
        for i, agent in enumerate(self.agents):
            ...
            # [修改点 3]: 加入目标航向向量和目标速度归一化
            # 将 target_yaw 转为 cos 和 sin 两个分量，保证神经网络输入的连续性
            target_yaw_vec = np.array([np.cos(self.target_yaw), np.sin(self.target_yaw)])
            
            # 归一化目标速度。假设场景中最大可能速度设为 12.0，将其映射到 [-1, 1] 左右的区间
            norm_target_speed = np.array([(self.target_speed / 12.0) * 2.0 - 1.0])

            # 拼接所有处理好的特征 (总量为: 2+2+1+2+6+16+2+1 = 32维)
            obs_dict[agent] = np.concatenate(
                [norm_pos, yaw_vec, norm_speed_scaled, rel_target, rel_mates, radar_data_scaled, target_yaw_vec, norm_target_speed]
            ).astype(np.float32)
    ...
    def step(self, actions_dict):
        ...
        for i, agent in enumerate(self.agents):
            ...
            new_tpos, t_hit = self._move_with_clip(self.target_position, self.target_yaw, self.target_speed,
                                                   self.target_radius)
        ...
        # ==================== 3. 阿波罗尼奥斯圆合围特征计算 ====================
        vecs = self.uav_positions - self.target_position
        angles = np.arctan2(vecs[:, 1], vecs[:, 0]) 
        
        sorted_indices = np.argsort(angles)
        sorted_angles = angles[sorted_indices]
        sorted_speeds = self.uav_speeds[sorted_indices]

        gaps = np.zeros(self.num_agents)
        for i in range(self.num_agents):
            next_i = (i + 1) % self.num_agents
            diff = sorted_angles[next_i] - sorted_angles[i]
            if diff < 0:
                diff += 2 * np.pi 
            gaps[i] = diff

        theta_min = np.min(gaps)
        ideal_angle = 2 * np.pi / self.num_agents
        r_p = np.exp(-abs(theta_min - ideal_angle))

        alpha_angles = np.zeros(self.num_agents)
        for i in range(self.num_agents):
            lam = sorted_speeds[i] / max(self.target_speed, 1e-3)
            lam = np.clip(lam, 0.0, 0.999) 
            alpha_angles[i] = np.arcsin(lam)

        escape_angles = np.zeros(self.num_agents)
        for i in range(self.num_agents):
            next_i = (i + 1) % self.num_agents
            gap_unblocked = gaps[i] - alpha_angles[i] - alpha_angles[next_i]
            escape_angles[i] = max(0.0, gap_unblocked)

        sum_escape_angle = np.sum(escape_angles)
        r_e = np.exp(-sum_escape_angle)

        r_encircle_new = 0.5 * r_p + 0.5 * r_e
        ...
        for i, agent in enumerate(self.agents):
            ...
            r_course = r_encircle_new - self.last_encircle_potential
        ...
        self.last_encircle_potential = r_encircle_new
    ...
    def state(self):
        # [修改点 4]: 为了保持全局状态(state)与个体观测(obs)的信息一致，将 target_speed 也加入到了全局状态中
        return np.concatenate([self.uav_positions.flatten(), self.uav_yaws, self.uav_speeds, self.target_position,
                               [self.target_yaw], [self.target_speed]]).astype(np.float32)
