...
class UAVPursuitApolloniusObsEnv(RawMultiAgentEnv):
    def __init__(self, config):
        super(UAVPursuitApolloniusObsEnv, self).__init__()
    ...
    def _assign_target_points(self):
        """
        基于阿波罗圆、最大连续开阔扇区、边界安全限制与分配惯性抗震荡的动态分配算法。
        """
        ...
        # [核心提权]: 将追捕者的安全边距提前定义，作为雷达扫描的“虚拟墙壁”
        safe_margin = 100.0 
        
        # 2. 射线探测逃生路线
        num_rays = 72
        angles = np.linspace(0, 2 * np.pi, num_rays, endpoint=False)
        is_open = np.zeros(num_rays, dtype=bool)
        ...
        for idx, angle in enumerate(angles):
            ...
            # (A) 检测追捕者安全边界 (100m)
            # 把地图边界向内收缩 100 米，作为射线的物理碰撞点
            tx = (self.map_size - safe_margin - self.target_position[0]) / (dx + 1e-8) if dx > 0 else (safe_margin - self.target_position[0]) / (dx - 1e-8)
            ty = (self.map_size - safe_margin - self.target_position[1]) / (dy + 1e-8) if dy > 0 else (safe_margin - self.target_position[1]) / (dy - 1e-8)
            
            # 【细节防御】：因为逃逸机的排斥线是 50m，它可能已经进入了追捕者的 100m 禁区。
            # 此时如果有射线指向外部边界，算出的距离会是负数。我们需要将其强行置为 0，表示“此路绝对不通”。
            tx = max(0.0, float(tx))
            ty = max(0.0, float(ty))
            
            min_dist = min(min_dist, tx, ty)

            # (B) 检测建筑物 AABB
            for b in self.buildings:
                xmin, xmax, ymin, ymax = b
                tx1 = (xmin - self.target_position[0]) / (dx + 1e-8)
                tx2 = (xmax - self.target_position[0]) / (dx + 1e-8)
                ty1 = (ymin - self.target_position[1]) / (dy + 1e-8)
                ty2 = (ymax - self.target_position[1]) / (dy + 1e-8)

                tmin = max(min(tx1, tx2), min(ty1, ty2))
                tmax = min(max(tx1, tx2), max(ty1, ty2))

                if tmax >= 0 and tmin <= tmax and tmin > 0:
                    min_dist = min(min_dist, tmin)

            # (C) 评判缺口：如果射线碰壁的距离足够长，才算有效逃生缺口
            if min_dist > r_f * 1.2:
                is_open[idx] = True
        ...
        # [核心优化 1]: 设定追捕者的安全边距，防止引导点落入 100m 惩罚区
        safe_margin = 50.0 
        ...
        if len(open_indices) == 0:
            ...
            for i, sector in enumerate(sectors):
                ...
                if num_for_sector > 0:
                    ...
                    if num_for_sector == 1:
                        angles_to_use = [(start_angle + end_angle) / 2]
                    else:
                        angles_to_use = np.linspace(start_angle, end_angle, num_for_sector)
        ...
        for i, uav_pos in enumerate(self.uav_positions):
            ...
            for j, tp_pos in enumerate(target_points):
                ...
                base_cost = dist + 20.0 * angle_diff

                # [核心优化 2]: 引入分配惯性惩罚 (Consistency Penalty)
                consistency_penalty = 0.0
                if prev_guide_pos is not None:
                    # 计算当前候选引导点，距离上一步的引导点有多远
                    shift_dist = np.linalg.norm(tp_pos - prev_guide_pos)
                    # 如果偏移极大，说明算法想让这架无人机去防守另一个完全不同的缺口
                    # 我们加上 1.5 倍的偏移距离惩罚，强行压制这种“反复横跳”的冲动
    ...
    def render(self, mode="rgb_array", save_path=None):
        ...
        for i, agent in enumerate(self.agents):
            ...
            if agent in self.current_guide_points:
                ...
                ax.plot(gx, gy, marker='*', markersize=12, color=color, zorder=4)
