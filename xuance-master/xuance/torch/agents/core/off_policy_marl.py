            ...
            for i in range(self.n_envs):
                if all(terminated_dict[i].values()) or truncated[i]:
                    ...
                    if self.use_wandb:
                        ...
                        # =====================================================================
                        # [重点修改区]: 拦截子奖励，重组字典结构以实现 TensorBoard 图表分组
                        # =====================================================================
                    if "episode_sub_rewards" in info[i]:
                        for agent_name, sub_rewards in info[i]["episode_sub_rewards"].items():
                            for reward_name, reward_value in sub_rewards.items():
                                if self.use_wandb:
                                    metric_key = f"Train-SubRewards-{agent_name}/{reward_name}/rank_{self.rank}/env-{i}"
                                    episode_info[metric_key] = reward_value
                                else:
                                        # 巧用斜杠 '/' 将智能体名字作为大分类，各项奖励作为独立图表
                                    metric_key = f"Train-SubRewards-{agent_name}/{reward_name}/rank_{self.rank}"
