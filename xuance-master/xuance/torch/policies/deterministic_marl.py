...
class GraphCommLayer(Module):
    """Attention-based communication over agents."""
    ...
    def __init__(self,
        ...
        super(GraphCommLayer, self).__init__()
        self.alpha = alpha
        self.query = torch.nn.Linear(input_dim, hidden_dim, device=device)
        self.key = torch.nn.Linear(input_dim, hidden_dim, device=device)
        self.value = torch.nn.Linear(input_dim, hidden_dim, device=device)
        self.out = torch.nn.Linear(hidden_dim, input_dim, device=device)

    def forward(self, x: Tensor):
        # x shape: [batch, n_agents, feat_dim]
        q = self.query(x)
        k = self.key(x)
        v = self.value(x)
        logits = torch.matmul(q, k.transpose(-2, -1)) / (q.shape[-1] ** 0.5 + 1e-8)
        attn = torch.softmax(logits, dim=-1)
        msg = torch.matmul(attn, v)
        return x + self.alpha * self.out(msg)
...
class Independent_DDPG_Policy(Module):
    ...
    def __init__(self,
        ...
        if self.use_graph_module:
            graph_input_dim = self.actor_representation[self.model_keys[0]].output_shapes['state'][0]
            self.graph_comm = GraphCommLayer(graph_input_dim, self.graph_hidden_dim,
                                             alpha=self.graph_alpha, device=device)
            self.target_graph_comm = deepcopy(self.graph_comm)
        else:
    ...
    def _apply_graph_comm(self, state_dict: Dict[str, Tensor], agent_list: List[str], use_target: bool = False):
        if (not self.use_graph_module) or (self.graph_comm is None):
            return state_dict

        graph_module = self.target_graph_comm if use_target else self.graph_comm

        if self.use_parameter_sharing:
            key = agent_list[0]
            x = state_dict[key]
            if x.shape[0] % self.n_agents != 0:
                return state_dict
            batch_size = x.shape[0] // self.n_agents
            state_dict[key] = graph_module(x.reshape(batch_size, self.n_agents, -1)).reshape(x.shape[0], -1)
            return state_dict

        feat_dims = [state_dict[k].shape[-1] for k in agent_list]
        batch_sizes = [state_dict[k].shape[0] for k in agent_list]
        if len(set(feat_dims)) != 1 or len(set(batch_sizes)) != 1:
            return state_dict

        stacked = torch.stack([state_dict[k] for k in agent_list], dim=1)
        updated = graph_module(stacked)
    ...
    def forward(self, observation: Dict[str, Tensor],
        ...
        for key in agent_list:
            ...
            actor_states[key] = outputs['state']
